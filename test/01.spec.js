...
import { type, mapx, map, deepCopy as copy, hasProp, length, keys, values } from '../src'
import { isString, isNumber, isArray, isObject, isAOF, isUN, isNull, isUndefined, isFunction } from '../src'
import { compose, composeDown, curry } from '../src'
import { toArray, ifMatch, whileMatch } from '../src'
...
describe('copy', () => {
  mapx(
    list,
    rollTest(copy, 'copy', false, (a, b) => a === b))
})

describe('hasProp a', function() {
  mapx(
    list,
    rollTest(hasProp, 'has', 'a'))
})

describe('hasProp o', function() {
  mapx(
    list,
    rollTest(hasProp, 'has', 'o'))
})
...
describe('compose', function() {
  ...
  it('normal', () => assert.equal(res, 45))

  const res2 = composeDown(
    x => x *3,
    x => x +5
  )(10)
  it('inverse', () => assert.equal(res2, 35))
})

describe('curry', function() {
  const list = [1, 2, 3]
  const fun = (v, k) => v +v +k
  const _map = func => list => mapx(list, func)
